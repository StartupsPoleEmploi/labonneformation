<?php
	...
	if($keywords!==null && (strlen($keywords)>4 || is_numeric($keywords)))
	{
		$listeRaw=array();
		$json=Tools::getUrlContent(URL_API_TREFLE.'/idcc?'.http_build_query(array('q'=>$keywords)),null,2);
		$listeRaw=json_decode($json,true);

		if(!empty($listeRaw))
		{
			$result=array('key'=>$keywords,'list'=>array());
			foreach($listeRaw as $idccCode=>$data)
			{
				$label=sprintf('<div style="float:right;">%s</div>&nbsp;<span>%s</span>',highlight($idccCode,$keywords),highlight($data['convention collective'],$keywords));
				$result['list'][]=array('label'=>$label,'value'=>$idccCode);
			}
		}
	}
